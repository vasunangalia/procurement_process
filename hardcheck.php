  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        <li class="scroll"><a href="check.php"><span class="glyphicon glyphicon-user"></span>Software Request </a></li>
        <li class="scroll active"><a href="hardcheck.php"><span class="glyphicon glyphicon-user"></span>Hardware Request</a></li>
	...
	<?php
		...
		if(mysqli_num_rows($ab)==0){
			...
			while($row = mysqli_fetch_assoc($ab)) {
				echo "<tr><td>{$row1['pid']}</td><td>$sid</td><td>{$row['name']}</td><td>{$row['specification']}</td><td>{$row1['estcost']}</td><td>{$row1['date']}</td>";
				}
			}
			}
			echo "</table><br><br><br>";
...
<form method='post'>
	Package Id:<br>
	<?php
	echo "<input name=\"pid\" list=\"phid1\">
  <datalist id=\"phid1\">";	
  $pid1 = mysqli_query($conc,"SELECT pid FROM package WHERE sid=''");
  if(!$pid1)
  {
  echo "No query";
  }
...
{
$sel = mysqli_query($conc,"UPDATE package SET status='$status' WHERE pid='$pid' AND sid=''");
$sel1 = mysqli_query($conc,"UPDATE package SET response='$avail1' WHERE pid='$pid' AND sid=''");
$sel1 = mysqli_query($conc,"UPDATE package SET sent='$send1' WHERE pid='$pid' AND sid=''");
if(!$sel)
{
