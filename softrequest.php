  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="scroll"><a href="hardrequest.php"><span class="glyphicon glyphicon-user"></span>Hardware Request</a></li>
...
echo "<input placeholder=\"Software Id\" name=\"sid\" list=\"sid1\">
  <datalist id=\"sid1\">";	
  $hid1 = mysqli_query($conc,"SELECT id FROM software");
  if(!$hid1)
  {
  echo "No query";
  }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
	$sid = $_POST['sid'];
$count = $_POST['count'];
$check = mysqli_query($conc,"SELECT senderid,sid FROM package WHERE senderid='$username' AND sid='$sid'");
if(mysqli_num_rows($check)==0)
{
$cost1 = mysqli_query($conc,"SELECT estcost FROM software WHERE id='$sid'");
$ct = mysqli_fetch_assoc($cost1);
$cs = $ct['estcost'];
$cost = $count*$cs;

$in = "INSERT INTO package (senderid,sid,count,estcost,date) VALUES ('$username','$sid','$count','$cost',NOW())
ON DUPLICATE KEY UPDATE sid=values(sid), count=values(count), estcost=values(estcost)";
...
<table border=\"0\" style="width:100%">
	...
	<tbody>
	<?php
		$dat = mysqli_query($conc,"SELECT estcost,date FROM package WHERE senderid='$username' AND sid='$sid'");
		$ab = mysqli_query($conc,"SELECT id,name,license,version,estcost FROM software WHERE id='$sid'");
		if(mysqli_num_rows($ab)==0){
				echo "<tr><td>No Softwares present</tr></td>";
			}
		else{
			
			
			while($row = mysqli_fetch_assoc($ab) and $row1=mysqli_fetch_assoc($dat)) {
				echo "<tr><td>$username</td><td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['license']}</td><td>{$row['version']}</td><td>{$row1['estcost']}</td><td>{$row1['date']}</td>";
				}
			}
			echo "</table><br><br><br>";
...
{
echo "Request for this component is already sent";
}
}
?>
<table border=\"0\" style="width:100%">
	...
	<tbody>
	<?php
		$ab = mysqli_query($conc,"SELECT id,name,license,version,estcost FROM software");
		if(mysqli_num_rows($ab)==0){
				echo "<tr><td>No Softwares present</tr></td>";
			}
		else{
			...
			while($row = mysqli_fetch_assoc($ab)) {
				echo "<tr><td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['license']}</td><td>{$row['version']}</td><td>{$row['estcost']}</td>";
				}
			}
			echo "</table><br><br><br>";
